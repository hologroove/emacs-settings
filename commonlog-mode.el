...
(setq commonlog-highlights
      '(
        ...
        ("\\(cri\\|maj\\):[a-z_]*:" . font-lock-warning-face) ;;critical/major + block name
        ("min:[a-z_]*:" . font-lock-keyword-face) ;; minor + block name
        ("inf:[a-z_]*:" . font-lock-constant-face) ;;info + block name
        ("<[0-9]+\.[0-9]+\.[0-9]+>" . font-lock-type-face) ;;PIDs
        ("^[0-9:.TZ-]+[a-z_]+:[a-z_]+:\"[^\"]*\":\\([a-z_0-9]+\\)" . (1 font-lock-type-face)) ;;process names
        ("[0-9]+" . font-lock-function-name-face) ;;numeric constant
        ))
